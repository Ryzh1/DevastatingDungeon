...
public class ChortPowerUp : MonoBehaviour
{
    ...
    private Chort_Controller chort;
    ...
    void Start()
    {
        chort = gameObject.GetComponent<Chort_Controller>();

    }

    // If chort is dead creates a random number if below 20 then it is a power up.
    //Then creates another random number which decides which powerup to place.
    void Update()
    {
        if (chort.ChortCurrentHealth <= 0)
        {
