...
public class ZombiePowerUp : MonoBehaviour
{ 
    ...
    private Zombie_Controller zombie;
    ...
    void Start()
    {
        zombie = gameObject.GetComponent<Zombie_Controller>();
        
    }

    // If zombie is dead creates a random number if below 20 then it is a power up.
    //Then creates another random number which decides which powerup to place.
    void Update()
    {
        if(zombie.ZombieCurrentHealth <= 0)
        {
